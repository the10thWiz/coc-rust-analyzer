...
export class CargoWatchProvider implements Disposable {
  ...
  constructor() {
    ...
    this.codeActionDispose = languages.registerCodeActionProvider(
  ...
  public start() {
    if (this.cargoProcess) {
      workspace.showMessage(`Cargo Watch is already running`);
      return;
    }
    ...
    this.cargoProcess = child_process.spawn('cargo', ['watch', '-x', args], {
  ...
  public stop() {
    if (this.cargoProcess) {
      ...
      workspace.showMessage(`Cargo Watch is not running`);
    }
  }
  ...
  private async parseLine(line: string) {
    ...
    if (data.reason === 'compiler-artifact') {
      ...
      const diagnostics: Diagnostic[] = [...(this.diagnosticCollection!.get(fileUri) || [])];

      // If we're building multiple targets it's possible we've already seen this diagnostic
      const isDuplicate = diagnostics.some(d => areDiagnosticsEqual(d, diagnostic));
      if (isDuplicate) {
        return;
      }
